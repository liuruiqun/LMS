...
uint16 HalAdcRead (uint8 channel, uint8 resolution)
{
  ...
  */
  ...
  adctemp |= channel | resbits | HAL_ADC_REF_VOLT;//
   //adctemp |= channel | resbits | HAL_ADC_REF_AIN7;//
 ...
 *          Note that the ADC is "bipolar", which means the GND (0V) level is mid-scale.
 **************************************************************************************************/
uint16 HalDiffAdcRead (uint8 channel0,uint8 channel1, uint8 resolution)
{
  int16  reading = 0;
  ...
  */
  ...
  /* Enable channel */
  ADCCFG |= adcChannel0|adcChannel1;
  tempadcfg=adcChannel0|adcChannel1;
  switch(tempadcfg)
  {
    case 0x03:
        channel=HAL_ADC_CHN_A0A1;
      break;
    case  0x0c:
      channel=HAL_ADC_CHN_A2A3;
      break;
    case 0x30:
      channel=HAL_ADC_CHN_A4A5;
      break;
    case 0xc0:
      channel=HAL_ADC_CHN_A6A7;
      break;
  }
 ...
 // channel=HAL_ADC_CHN_A6A7;
  adctemp &= ~(HAL_ADC_CHN_BITS | HAL_ADC_DEC_BITS | HAL_ADC_REF_BITS);
  adctemp |= channel | resbits | HAL_ADC_REF_VOLT;//
 // adctemp |= channel | resbits | HAL_ADC_REF_125V ;//
 ...
 *
 **************************************************************************************************/
uint16 HalAdcReadVcc ( uint8 resolution)
{
  int16  reading = 0;
  ...
  /* Setup Sample */
  adctemp = ADCCON3;
  adctemp &= ~(HAL_ADC_CHN_BITS | HAL_ADC_DEC_BITS | HAL_ADC_REF_BITS);//0xFF
  adctemp |= HAL_ADC_CHN_VDD3 | resbits | HAL_ADC_REF_125V;//0xBF
  // adctemp |=  HAL_ADC_REF_AIN7 | resbits | HAL_ADC_CHN_VREF  ;
 ...
 * @fn      HalReadExtAdcVcc
 *
 * @brief   Read Externel P0.7 VCC value
 *
 * @param  
 *
 * @return 
...
uint16 HalReadExtVcc ( uint8 resolution)
{
  int16  reading = 0;
  ...
  /* Setup Sample */
  adctemp = ADCCON3;
  adctemp &= ~(HAL_ADC_CHN_BITS | HAL_ADC_DEC_BITS | HAL_ADC_REF_BITS);//0
  adctemp |=  HAL_ADC_REF_AIN7 | resbits | HAL_ADC_CHN_VREF  ;//0x0d |resbits| 0x40
