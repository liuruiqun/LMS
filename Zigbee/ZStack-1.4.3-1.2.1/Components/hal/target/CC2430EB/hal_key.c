...
static ISR_t P1_ISR_fun[8];
static uint8 P1InterruptTaskID[8];
static rain_t sensor_rain;
 ...
 * @return
 **************************************************************************************************/
void OpenP1Interrupt( uint8 pin, uint8 FunSelect, uint8 direction, uint8 InputMode, uint8 RiseFallEdge)
{
  if(FunSelect==FUN_SELECT_GENERAL)
  {
    P1SEL &= ~(1<<pin);
  } 
  else 
  ...
  if(direction==DIRECTION_INPUT)
  {
    ...
    if(InputMode==INPUT_MODE_PULLUP)
    {
      P1INP &= ~(1<<pin);
      P2INP &= ~(0x40);
    }
    else if (InputMode==INPUT_MODE_PULLDOWN)
    {
      P1INP &= ~(1<<pin);
      P2INP |= (0x40);
    } 
    else if (InputMode==INPUT_MODE_TRISTATE)
    {
      P1INP |= (1<<pin);
    }
    
    if(RiseFallEdge==RISE_EDGE)
    {
      PICTL &= ~(0x02);
    }
    else
  ...
  {
    case PIN_0:
      P1_ISR_fun[pin] = P1_0_ISR_fun;
      break;
    case PIN_1:
      P1_ISR_fun[pin] = P1_1_ISR_fun;
      break;
    case PIN_2:
      P1_ISR_fun[pin] = P1_2_ISR_fun;
      break;
    case PIN_3:
      P1_ISR_fun[pin] = P1_3_ISR_fun;
      break;
    case PIN_4:
      P1_ISR_fun[pin] = P1_4_ISR_fun;
      break;
    case PIN_5:
      P1_ISR_fun[pin] = P1_5_ISR_fun;
      break;
    case PIN_6:
      P1_ISR_fun[pin] = P1_6_ISR_fun;
      break;
    case PIN_7:
      P1_ISR_fun[pin] = P1_7_ISR_fun;
      break;
  }
}
 ...
 * @return
 **************************************************************************************************/
void CloseP1Interrupt(uint8 pin)
{
  P1_ISR_fun[pin]=0;
}
 ...
 * @return
 **************************************************************************************************/
void StartP1Interrupt(uint8 pin)
{
  EA = 1;
  IEN2 |= (0x10);
  P1IEN |= (1<<pin);
 ...
 * @return
 **************************************************************************************************/
void StopP1Interrupt(uint8 pin)
{
  P1IEN &= ~(1<<pin);  
}

/**************************************************************************************************
 * @fn      RegisterP1Interrupt
 *
 * @brief   
 *
 * @param
 ...
 * @return
 **************************************************************************************************/
void RegisterP1Interrupt(uint8 pin, uint8 TaskID)
{
    P1InterruptTaskID[pin]=TaskID;
}
 ...
 * @return
 **************************************************************************************************/
uint8 QueryP1Interrupt(uint8 pin)
{
  switch(pin)
  {
    case PIN_0:
    ...
    case PIN_2:
      break;
    case PIN_3:
      sensor_rain.ReadState=1;
      return sensor_rain.count;      
      //break;
    case PIN_4:
 ...
 * @return
 **************************************************************************************************/
void HalP1InterruptConfig( void )
{
  EA=1;
  uint8 pin;
  for(pin=0;pin<8;pin++)
  {
    P1_ISR_fun[pin]=0;
    P1InterruptTaskID[pin]=0xFF;  //0xFF is not register
  }
  EA=0;  
}
...
HAL_ISR_FUNCTION( halP1Isr, P1INT_VECTOR )
{
  ...
  for(pin=0;pin<8;pin++)
  {
    if((P1IFG & (1<<pin)) && P1_ISR_fun[pin])
    {
      P2_0=!P2_0;           //LED -- test purpose      
      P1IFG &= ~(1<<pin);   //clean
      (P1_ISR_fun[pin])();
    }
  }
...
{
  sensor_rain.count++;
  sensor_rain.ReadState=0;
  if(P1InterruptTaskID[3]!=0xFF)
  {
    rain_t *msgPtr;
    msgPtr = (rain_t *)osal_msg_allocate( sizeof(rain_t) );
    if ( msgPtr )
    {
      msgPtr->hdr.event = P1_3_INTERRUPT;
      msgPtr->count = sensor_rain.count;
      osal_msg_send(P1InterruptTaskID[3], (uint8 *)msgPtr);         
      sensor_rain.ReadState=1;
    }
  }  
}
