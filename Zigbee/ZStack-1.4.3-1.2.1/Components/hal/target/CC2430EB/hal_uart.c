...
uint8 HalUARTOpen( uint8 port, halUARTCfg_t *config )
{
  ...
  *cfgPP = (uartCfg_t *)osal_mem_alloc( sizeof( uartCfg_t ) );
  ...
  if ( port == HAL_UART_PORT_0 )
  {
    // Only supporting 38400 or 9600 for code size - other is possible.
    U0BAUD = (config->baudRate == HAL_UART_BR_38400) ? 59 : 59;
    U0GCR = (config->baudRate == HAL_UART_BR_38400) ? 10 : 8;
  ...
  if ( port == HAL_UART_PORT_1 )
  {
    ...
    U1BAUD = (config->baudRate == HAL_UART_BR_38400) ? 59 : 59;
    U1GCR = (config->baudRate == HAL_UART_BR_38400) ? 10 : 8;
...
/**************************uart1***********************************/ 
   ...
   // pollDMA( cfg );
     ...
     */
    /************************uart0********************/
      if ( mycfg0->rxHead != mycfg0->rxTail )
      {
      uint8 evt0;

      if ( mycfg0->rxHead >= (mycfg0->rxMax - SAFE_RX_MIN) )
      {
        evt0 = HAL_UART_RX_FULL;
      }
      else if ( mycfg0->rxHigh && (mycfg0->rxHead >= mycfg0->rxHigh) )
      {
        evt0 = HAL_UART_RX_ABOUT_FULL;
      }
      else if ( mycfg0->rxTick == 0 )
      {
        evt0 = HAL_UART_RX_TIMEOUT;
      }
      else
      {
        evt0 = 0;
      ...
      if ( evt0 && mycfg0->rxCB )
      {
          mycfg0->rxCB( ((mycfg0->flag & UART_CFG_U1F)!=0), evt0 );
      }
     }
     /************************uart1********************/
      if ( mycfg1->rxHead != mycfg1->rxTail )
      {
      uint8 evt1;

      if ( mycfg1->rxHead >= (mycfg1->rxMax - SAFE_RX_MIN) )
      {
        evt1 = HAL_UART_RX_FULL;
      }
      else if ( mycfg1->rxHigh && (mycfg1->rxHead >= mycfg1->rxHigh) )
      {
        evt1 = HAL_UART_RX_ABOUT_FULL;
      }
      else if ( mycfg1->rxTick == 0 )
      {
        evt1 = HAL_UART_RX_TIMEOUT;
      }
      else
      {
        evt1 = 0;
      ...
      if ( evt1 && mycfg1->rxCB )
      {
          mycfg1->rxCB( ((mycfg1->flag & UART_CFG_U1F)!=0), evt1 );
      }
     } 
/*
#if HAL_UART_0_ENABLE
