...
// Handle incoming ZCL message
typedef struct
{
  zclIncoming_t *msg;                // incoming message
  uint16        logicalClusterID;    // logical cluster ID
} zclInHdlrMsg_t;
...
typedef ZStatus_t (*zclInHdlr_t)( zclInHdlrMsg_t *pInHdlrMsg );
 ...
 */
extern ZStatus_t zcl_registerAttrList( uint8 endpoint, uint8 numAttrs, CONST zclAttrRec_t attrList[] );
 ...
 */
extern ZStatus_t zcl_SendDefaultRspCmd( uint8 srcEP, afAddrType_t *dstAddr,
