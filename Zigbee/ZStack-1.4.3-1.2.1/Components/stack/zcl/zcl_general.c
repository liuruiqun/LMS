 ...
 * LOCAL FUNCTIONS
 */
static ZStatus_t zclGeneral_HdlIncoming( zclInHdlrMsg_t *pInHdlrMsg );
                      ...
                      uint16 identifyTime, uint8 disableDefaultRsp, uint8 seqNum )
{
  ...
  return zcl_SendCommand( srcEP, dstAddr, ZCL_GEN_LOGICAL_CLUSTER_ID_IDENTIFY,
                          TRUE, COMMAND_IDENTIFY, TRUE, ZCL_FRAME_CLIENT_SERVER_DIR,
                 ...
                 uint8 status, aps_Group_t *grp, uint8 disableDefaultRsp, uint8 seqNum )
{
  ...
  if ( buf )
  {
    ...
    stat = zcl_SendCommand( srcEP, dstAddr, ZCL_GEN_LOGICAL_CLUSTER_ID_GROUPS, 
                            TRUE, COMMAND_GROUP_VIEW_RSP, TRUE, ZCL_FRAME_SERVER_CLIENT_DIR,
                            disableDefaultRsp, 0, seqNum, len, buf );
    osal_mem_free( buf );
  }
  else
  ...
  if ( buf )
  {
    ...
    *pBuf++ = scene->ID;
    ...
    stat = zcl_SendCommand( srcEP, dstAddr, ZCL_GEN_LOGICAL_CLUSTER_ID_SCENES,
                            TRUE, COMMAND_SCENE_VIEW_RSP, TRUE, ZCL_FRAME_SERVER_CLIENT_DIR, 
                            disableDefaultRsp, 0, seqNum, len, buf );
    osal_mem_free( buf );
  }
  else
                              ...
                              uint32 timeStamp, uint8 disableDefaultRsp, uint8 seqNum )
{
  ...
  return zcl_SendCommand( srcEP, dstAddr, ZCL_GEN_LOGICAL_CLUSTER_ID_ALARMS,
                          TRUE, COMMAND_ALARMS_GET_RSP, TRUE, ZCL_FRAME_SERVER_CLIENT_DIR, 
                          disableDefaultRsp, 0, seqNum, len, buf );
}
#endif // ZCL_ALARMS
...
static ZStatus_t zclGeneral_HdlIncoming( zclInHdlrMsg_t *pInHdlrMsg )
{
  ...
  if ( zcl_ClusterCmd( pInHdlrMsg->msg->hdr.fc.type ) )
  {
    ...
    {
      stat = zclGeneral_HdlInSpecificCommands( pInHdlrMsg->msg, pInHdlrMsg->logicalClusterID );
    }
    else
    {
...
static ZStatus_t zclGeneral_ProcessInIdentity( zclIncoming_t *pInMsg )
{
  ...
  if ( zcl_ServerCmd( pInMsg->hdr.fc.direction ) )
  {
    ...
    else
    {
      ...
      if ( zclFindAttrRec( pInMsg->msg->endPoint, ZCL_HA_CLUSTER_ID_GEN_IDENTIFY, ATTRID_IDENTIFY_TIME, &attrRec ) )
  ...
  else // Client Command
  {
    ...
    pCBs = zclGeneral_FindCallbacks( pInMsg->msg->endPoint );
    if ( pCBs && pCBs->pfnIdentifyQueryRsp )
    {
...
static ZStatus_t zclGeneral_AddGroup( uint8 endPoint, aps_Group_t *group, uint8 *pData )
{
  ...
  if ( zclFindAttrRec( endPoint, ZCL_HA_CLUSTER_ID_GEN_GROUPS, ATTRID_GROUP_NAME_SUPPORT, &attrRec ) )
...
static ZStatus_t zclGeneral_ProcessInScenesServer( zclIncoming_t *pInMsg )
{
  ...
  switch ( pInMsg->hdr.commandID )
  {
    case COMMAND_SCENE_ADD:
      ...
      // Retrieve Name Support attribute
      if ( zclFindAttrRec( pInMsg->msg->endPoint, ZCL_HA_CLUSTER_ID_GEN_SCENES, ATTRID_SCENES_NAME_SUPPORT, &attrRec ) )
        zclReadAttrData( &nameSupport, &attrRec );
